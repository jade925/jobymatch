    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        <div className="flex items-center justify-between mb-5">
          <button onClick={prevMonth} className="p-2"><ChevronLeft size={22} style={{ color: "#393E41" }} /></button>
          <h2 className="font-heading text-lg" style={{ color: "#393E41" }}>{MONTHS[month]} {year}</h2>
          <button onClick={nextMonth} className="p-2"><ChevronRight size={22} style={{ color: "#393E41" }} /></button>
        </div>

        <div className="grid grid-cols-7 mb-2">
          {DAYS_HEADER.map((d, i) => (
            <div key={i} className="text-center text-xs font-sans py-1" style={{ color: "#9ca3af" }}>{d}</div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-y-1">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div
                key={day}
                onClick={() => { setSelectedDate(dateStr); setSheet(true); }}
        ...
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl p-5 pb-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setSheet(false)}><X size={20} style={{ color: "#9ca3af" }} /></button>
            </div>
              ...
              <div className="space-y-3">
                {selectedEvents.map((ev) => (
                  <div key={ev.id} className="rounded-xl p-4 border" style={{ borderColor: "#e2e3d8", backgroundColor: "#F6F7EB" }}>
                    <p className="font-heading text-sm" style={{ color: "#393E41" }}>{ev.match.offer.title}</p>
                    <p className="font-sans font-light text-sm mt-0.5" style={{ color: "#6b7280" }}>
                      {ev.match.student.firstName} {ev.match.student.lastName}
                    </p>

...
export default function CompanyCalendarPage() {
  ...
  useEffect(() => {
    setMatches(getMatches().filter((m) => m.status === "ACCEPTE"));
  }, []);
  ...
  const eventDates = new Set(matches.map((m) => m.appliedAt.slice(0, 10)));
  ...
  const selectedMatches = matches.filter(
    (m) => m.appliedAt.slice(0, 10) === selectedDate
  );
